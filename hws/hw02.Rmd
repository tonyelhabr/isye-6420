...
<hide>
https://www.symbolab.com/solver/integral-calculator/%20%5Cint_%7Bx%7D%5E%7B%5Cinfty%7D%203%5E%7B2%7Dexp%5Cleft(-3y%5Cright)dy
</hide>
...
$$
\begin{array}{lcll}
f_X(x) & = & \int_{0}^{y} \lambda^2 e^{-\lambda y} dy \\
& = & \lambda^2 \int_{0}^{y} e^{-\lambda y} dy \\
& = & \lambda^2 \int_{0}^{-\lambda y} e^{u} du & \text{where }u= -\lambda y \\
& = & \lambda^2 (\frac{1}{-\lambda} e^{u} \Big|_{0}^{-\lambda y}) \\
& = & -\lambda (e^{-\lambda y} - 1) \\
& = & \lambda (1 - e^{-\lambda y}) \\
& = & \frac{\lambda  - 1}{e^{\lambda y}} \\
\end{array}
$$
...
<hide>
**FIXME**:
https://www.symbolab.com/solver/integral-calculator/%5Cint_%7Bx%7D%5E%7B1%7D%20C%5E%7B2%7De%5E%7B-Cy%7D%20dx
</hide>
...
$$
\begin{array}{lcl}
f_Y(y) & = & \int_{x}^{?} \lambda^2 e^{-\lambda y} dx \\
& = & \lambda^2 e^{-\lambda y} \int_{x}^{?} dx \\
\end{array}
$$
...
$$
\begin{array}{lcl}
f(x) & = & \frac {\lambda^{2}x^{2-1}e^{-\lambda x}}{\Gamma(2)} \\
& = & \frac {\lambda^{2} x e^{-\lambda x}}{1} \\
& = & \lambda^{2} x e^{-\lambda x} \sim \mathcal{G}a(2,\lambda)
\end{array}
$$
