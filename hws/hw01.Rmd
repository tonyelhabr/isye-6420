...
.path_sans_ext <- file.path('hws', 'hw01')
...
Figure 1: Components $E_1,\dots E_4$ at operational at time $t$ with probabilities $e^{−t}, e^{−2t}, e^{−t/2}, e^{−t/3}$ and $e^{−t}$, respectively.
...
Note that we could have also started from the following formulation:
$P(S|H_1) = P(E_1) + P(E_3) - P(E_1 \cap E_3)$
...
See the implementation in `R` below.
...
```
And, finally, see the calculation of the the probability that the system $S$ will be operational at time $t = `r t_x_hide`$.
...
<response>
Below is plot of this probability as a function of time $t$ for $0 \leq t \leq 10$.
</response>

```{r hw01-q01-a-5, include=T, echo=F, eval=T}
seq_t <- seq(0, 10, by = 0.1)
d_q1 <- tibble(t = seq_t)
...
We could implement this in `R` as follows.
...
<instructions>
Hint: This is both Bayes rule and Total Probability. The two hypotheses concern the type
of batch. For the first draw they are equally likely (batch is selected at random), but for the
second draw, they are updated by the information on first draw via Bayes rule. Updated
probabilities of hypotheses are used in the total probability formula.
</instructions>
...
Note that the problem description suggests that
$P_0(B_1) = P_0(B_2) = 0.5, P(C=1|B_1)=1, P(C=1|B_2)=0.8$,
where the subscript $0$ in $P_0$ indicates that the probability is a prior probability.
Then, we have the following. (Let $1$ in $P_1$ indicate the probability is a posterior probability.)

$$
\begin{array}{lcl}
P_1(B_1) & = & \frac{P(C=1|B_1) P(B_1)}{P(C=1|B_1) P(B_1) + P(C=1|B_2) P_0(B_2)} \\
& = & \frac{(1)(0.5)}{(1)(0.5)+(0.8)(0.5)} \\
& = & \frac{0.5}{0.5+0.4} = \frac{5}{9} \approx 0.555 \\
\end{array}
$$

Then we can calculate $P_1(C=1)$ as follows.

$$
\begin{array}{lcl}
P_1(C=1) & = & P(C=1|B_1) P_1(B_1) + P(C=1|B_2) P_1(B_2) \\
& = & P(C=1|B_1) P_1(B_1) + P(C=1|B_2) (1 - P_1(B_1)) \\
...
<response>
Then $P_1(C=0) = 1 - P_1(C=1) = \frac{4}{45} \approx 0.089$.
</response>
...
an item is selected at random. What is the probability that the item is of label 1, if the classifier says it was.
</instruction>
...
Given the problem setup, the cells of the table are as follows. (The instruction-provided numbers are emphasized. The other numbers represent "calculations".)
...
```{r hw01-q3-1, include=F, echo=F, eval=F}
sum_y_neg <- 50
sum_y_pos <- 70
sum_x_neg <- 55
sum_x_pos <- 65
...
sum_pop <- sum_x_neg + sum_x_pos
sum_pop
# Or...
sum_pop <- sum_y_neg + sum_y_pos
sum_pop
fp <- sum_y_pos - tp
fp
fn <- sum_x_pos - tp
...
```{r hw03-q3-2, include=F, echo=F, eval=F}
sum_x_pos_new <- 0.01 * sum_pop
sum_x_pos_new
sum_x_neg_new <- sum_pop - sum_x_pos_new
sum_x_neg_new
```

We are interested in the positive predictive value (PPV) (also know as the 
precision), which is the ratio of true positives to the sum of predicted positives.
(i.e. $PPV = TP / (TP + FP)$). Here, $PPV = TP / (TP + FP) = \frac{52}{(52 + 18)} = \frac{26}{35} \approx 0.743$.

Note that we can arrive at the same number using a more "Bayesian approach".

$$
\begin{array}{l}
P(Y=1|X=1) = \frac{52}{65} = \frac{4}{5}, \\ P(X=1)=\frac{65}{120}=\frac{13}{24}, P(Y=1|X=0)=\frac{18}{55}, \\ P(X=0)=\frac{55}{120}=\frac{11}{24}
\end{array}
$$.
...
$$
A = P(Y=1|X=1) P(X=1) = (\frac{4}{5}) (\frac{13}{24}) =\frac{13}{30}=0.433
$$

and

$$B = P(Y=1|X=0) P(X=0) = (\frac{18}{55}) (\frac{11}{24}) = \frac{3}{20}=0.15$$.

Then we have the following.

$$
\begin{array}{lcl}
P(X=1|Y=1) & = & \frac{P(Y=1|X=1) P(X=1)}{P(Y=1|X=1) P(X=1) + P(Y=1|X=0) P(Y=1|X=0))} \\
& = & \frac{A}{A+B} \\
& = & \frac{13/30}{13/30+3/20} \\
& = &  \frac{26}{35} \approx 0.743 
\end{array}
$$

<response>
This probability stays true in the case that the proportion of 0's and 1's is
modified, so we can say that, given that the classifiers says the item is of label 1,
the probability that the item is truly of label 1 is 0.743.
</response>
