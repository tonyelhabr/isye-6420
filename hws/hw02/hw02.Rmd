...
$T_1, \dots, T_n \sim \mathcal{E}(\lambda)$ given the prior 
$\pi(\lambda) = \frac{1}{\sqrt{\lambda}}, \lambda > 0$. 

$$
\begin{array}{c}
X_1, X_2, X_3 \sim \mathcal{E}(\lambda)
\end{array}
$$

$$
\begin{array}{c}
\sum_{i=1}^{n=3} X_i \sim \mathcal{Ga}(n=3, \lambda)
\end{array}
$$
...
$$
\begin{array}{rclcl}
\text{ Likelihood } & : & x | \lambda & \sim & \lambda^n e^{-\lambda \sum_{i=1}^{n=3} x_i} \\
\text{ Prior } & : & \lambda & \sim & \frac{1}{\sqrt{\lambda}} \\
\text{ Posterior } & : & \lambda | x & \sim & 
\end{array}
$$
