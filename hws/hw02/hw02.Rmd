...
<hide>
Best reference:
+ PDF page 50 of http://springer.bme.gatech.edu/solbook.pdf
</hide
...
& \propto & \lambda^{n-0.5} e^{-\theta \sum_{i=1}^{n} t_i} \\
& \propto & \lambda^{(n+0.5)-1} e^{-\theta \sum_{i=1}^{n} t_i}.
\end{array}
$$
...
We can go further with this result and calculate the posterior mean and variance.

$$
\begin{array}{rcl}
\operatorname{E}[T|\alpha, \beta] & = & \frac{\alpha}{\beta} = \frac{3.5}{24} = \frac{7}{48} \approx 0.1458. \\
\operatorname{Var}(T|\alpha, \beta) & = & \frac{\alpha}{\beta^2} = \frac{3.5}{24^2} \approx 0.0061.
\end{array}
$$

We note that the posterior mean is relatively close to the MLE estimate $\frac{1}{8}$.
