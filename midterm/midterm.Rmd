...
$$
\begin{array}{l}
\Pr_{1} & = & \Pr(\text{vaca=yes|car=yes}) \times \Pr(\text{car=yes|market=bull}) \times \sum_g^{a,b,c} \Pr(\text{car=yes|grade=g}) \\
\Pr_{2} & = & \dots,
\end{array}
$$
...
Now, to begin filling in numbers for our Bayes' equation above, let's begin by finding $\Pr(\text{vaca|yes})$. Filtering for just the `vaca == 'yes'` scenarios, we find that ther are `r nrow(dplyr::filter(states_wo_lott, vaca == 'yes'))` records.
...
We want to find $\Pr(\text{lott=yes|vaca=yes})$. This can be calculated using Bayes' theorem as follows.

$$
\begin{aligned}
\Pr(\text{lott=yes|vaca=yes}) &
= \frac{\Pr(\text{vaca=yes|lott=yes}) \Pr(\text{lott=yes})}{\Pr^{\prime}(\text{vaca=yes})}
\end{aligned}
$$

We are already given two of the quantities that we'll need---namely, the terms in the numerator, $\Pr(\text{vaca=yes|lott=yes}) = 0.99$ and $\Pr(\text{lott=yes}) = 0.001$. The denominator term $\Pr^{\prime}(\text{vaca=yes})$ is equivalent to the sum of the product of the numerator terms and $\Pr(\text{vaca=yes})$ found in part a. (Note the distinction of the $\Pr(\text{vaca=yes})$ from part a and the one here with the prime symbol $^{\prime}$.)
...
\Pr(\text{lott=yes|vaca=yes})
&= \frac{\Pr(\text{vaca=yes|lott=yes}) \Pr(\text{lott=yes})}{\Pr^{\prime}(\text{vaca=yes})} \\
&= \frac{\Pr(\text{vaca=yes|lott=yes}) \Pr(\text{lott=yes})}{\Pr(\text{vaca=yes|lott=yes}) \Pr(\text{lott=yes}) + \Pr(\text{vaca=yes})} \\
...
The following code implements all parts of the instructions. Specifically `compute_q2()` calls other functions to compute the required components---(1) `compute_beta_mu()` for $p_{\text{Bayes}}$, `compute_equi_credible_set_b()` for the 95% credible set, and (3) `compute_p_h1()` for the hypothesis test. Discussion of the findings for each set of priors is included in the following subsections.
...
mcmc_gibb_mu_mean_chr <- format_num(mcmc_gibb_mu_mean, digits = 4)
mcmc_gibb_mu_mean_chr

y_mean_chr <- format_num(y_mean, digits = 0)
y_mean_chr
```

We find that $\hat{\mu}$ = `r mcmc_gibb_mu_mean_chr`, which is between the observed mean $\hat{y}$ = `r y_mean_chr` and the prior $\mu_0$ = `r mu_0`. This is what we should have expected.
...
p_h_0 <- h_0 %>% pull(frac)
p_h_0
p_h_0_chr <- p_h_0 %>% format_num(3)
p_h_0_chr
p_h_0_pct_chr <- (p_h_0 * 100) %>% format_num(1)
p_h_0_pct_chr
```
<response>
We find that the posterior probability is $p_{H_{0}}$ = `r p_h_0_chr` (i.e. `r p_h_0_pct_chr`%).
...
b_01_log10_chr <- format_num(b_01_log10)
b_01_log10_chr
```

We deduce that there is strong evidence against the alternative hypothesis $H_1 \geq$ `r mu_split`---or equivalently, in favor of $H_0$---because $1.5 < \log_{10}(B_{01})$ = `r b_01_log10_chr` $\leq 2$.


### b

We can calculate the 95% equitailed credible set for $\tau$ as follows.
