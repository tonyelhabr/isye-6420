...
<response>
The slope (i.e. the posterior estimate of the mean) of $\beta_2$ (for $\text{mage}$) is and the 95% CS of the mean response for the new store data 
</response>
...
<response>
From the output shown above, we see that the slope (i.e. the posterior estimate of the mean) of $\beta_2$ (for $\text{death}$) is `r b_2_q1`. Its 95% CS is [`r cs_b_2_q1[1]`, `r cs_b_2_q1[2]`]. This variable is significant because its 95% CS is completely negative.
</response> 
...
<response>
From the output shown above, we see that the predicted time between births for Helga (corresponding to `pred1`) is `r est_pred1_q1` with 95% CS is [`r cs_pred1_q1[1]`, `r cs_pred1_q1[2]`].
...
</response> Notably, this CS is very wide. It even includes zero (which does not make sense in the context of this problem)! Thus, we should be wary of the lack of robustness of this prediction.

### Aside

As an aside, we can create the same kind of model using a "traditional" (i.e. non-Bayesian) approach in `R`. Overall, we achieve similar results. 

+ The estimates of all $\beta$'s are similar. Specifically, we note that $\beta_1$ and $\beta_2$ are estimated to be negative, as is found above.

+ The prediction for Helga is in the 900s, as us found in the (c); and the prediction intervals for both Helga and Emma have wide 95% prediction intervals, as is found above.
...
+ The three `season` comparison terms involving "Winter" are significantly positive: (1) `cb[1, 4]` (i.e. the difference between "Spring" and "Winter"); (2) `cb[2, 4]` (i.e. the difference between "Summer" and "Winter"); (3) `cb[3, 4]` (i.e. the difference between "Autumn" and "Winter"). We may say that 

+ Two terms have negative posterior means but are not significant: `ca[1, 2]` and `cb[1, 2]`.
 
+ The remaining two terms---involving `season` comparisons with "Fall"---have positive posterior means but are not significant: `cb[1, 3]` and `cb[2, 3]`.
...
+  For our implementation, we define $\tau = 0.01$ (shown in the stochastic process equations above) so that the $\alpha_i$ estimates are relatively noninformative. (This maintains the premise that we should not make our priors "too noninformative".)
...
<response>
From the output shown above, we see that the mean response is `r pred1_mean_q3c` with 95% CS is [`r cs_pred1_mean_q3c[1]`, `r cs_pred1_mean_q3c[2]`]. The prediction is `r pred1_q3c` with 95% CS is [`r cs_pred1_q3c[1]`, `r cs_pred1_q3c[2]`].
