...
```{r viz_summ_1_z_adj-show, include=T}
...
```{r viz_conf_mat_export, eval=eval_viz_preds}
export_png(
  viz_conf_mat,
...
```{r viz_conf_mat-show, include=T}
knitr::include_graphics(path_viz_conf_mat)
```

I can make a couple of observations:

+ I see that the most common outcome of the join actual-prediction possibilities is an actual win by the home team and a predicted win by the home team.
+ The model predicts the outcome correctly in `r scales::comma(pull(conf_mat_corrects_h, n)` + `r scales::comma(pull(conf_mat_correct_t, n)` + `r scales::comma(pull(conf_mat_correct_a, n)` = `r scales::comma(pull(conf_mat_correct_summ_yes, n))` of `r scales::comma(n_gm)` games (i.e., `r scales::percent(pull(conf_mat_correct_summ_yes, frac))`).
...
```{r viz_g_mode-show, include=T}
knitr::include_graphics(path_viz_g_mode)
```
...
```{r viz_g_mean-show, include=T}
knitr::include_graphics(path_viz_g_mean)
```

The above histogram illustrating the average (instead of the mode) provides a bit more nuance to the predicted goals scored by each side.
...
```{r viz_result_mode_export, eval=eval_viz_preds}
export_png(
  viz_result_mode,
...
```{r viz_result_mode-show, include=T}
knitr::include_graphics(path_viz_result_mode)
```

Finally, the above illustration of results illustrates the predicted outcomes (inferred from the prior graph of predicted modes). *This is the primary input to the confusion matrix from before.)

To better understand how the model works on a team-level basis, we can look at how well it predicts for each team.
...
We can take an even closer look at any team of interest. For example, let's look exclusively at the predictions and actual outcomes for `r .tm1` (the highest rated team by the model.)


```{r preds_eda-appendix-2}
.tm1 <- 'Man City'
```

```{r preds_eda-appendix-3}
preds_by_tm_1tm <-
  preds_aug %>% 
  filter(tm_h == .tm1 | tm_a == .tm1) %>% 
  count(is_correct = result_mode == result) %>% 
  mutate(frac = n / sum(n))
```

```{r preds_eda-show-2, include=T}
preds_by_tm_1tm
```
